<?php 
include 'header.php'; 
$urunsor=$db->prepare("SELECT * FROM kategori where kategori_id=:id");
$urunsor->execute(array(
  'id' => $_GET['id']
  ));

$uruncek=$urunsor->fetch(PDO::FETCH_ASSOC);
$kategorisorsa=$db->prepare("SELECT * FROM kategori where kategori_id=:id");
$kategorisorsa->execute(array(
  'id' => $_GET['id']
  ));

$kategoricekse=$kategorisorsa->fetch(PDO::FETCH_ASSOC);
...
header("Location:../index");
    exit;
                     ...
                     $sorgu=$db->prepare("SELECT * FROM urun where kategori_id={$_GET['id']}");
                     $sorgu->execute();
 ...
 $urunsor=$db->prepare("SELECT * FROM urun where kategori_id={$_GET['id']} order by urun_id DESC limit $limit,$sayfada");
        $urunsor->execute();
...
?>
        ...
        <section class="banner-section page-banner bg-gra4">
            ...
            <div class="banner-content-area">
                <div class="container">
                    <?php 
                    $kategorisorr=$db->prepare("SELECT * FROM urun where kategori_id={$_GET['id']}");
                    $kategorisorr->execute();
                    $kategoricekk=$kategorisorr->fetch(PDO::FETCH_ASSOC);
                    $kategorisor2=$db->prepare("SELECT * FROM kategori where kategori_id={$kategoricekk['kategori_id']}");
                     $kategorisor2->execute();
                     $kategoricek2=$kategorisor2->fetch(PDO::FETCH_ASSOC);
                     ?>
                    <div class="banner-text text-center text-lg-left">
                        <h1 class="title"><?php echo $kategoricek2['kategori_isim']; ?></h1>
            ...
            <div class="breadcrumb-area">
                <div class="container d-flex justify-content-end">
                    <nav aria-label="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page"><?php echo $kategoricek2['kategori_isim']; ?></li>
                        </ol>
                    </nav>
                </div>
            </div>
...
$kategorisor->execute();
  ...
  <ul class="pagination justify-content-center">
    ...
    <li class="page-item active"><a class="page-link" onclick="javascript:location.href='<?php echo $ayarcek['ayar_url']; ?>kategori/<?=seo($kategoricekse['kategori_isim']).'/'.$kategoricekse["kategori_id"]?>?sayfa=<?php echo $s; ?>';"><?php echo $s; ?></a>

    </li>
     <?php } else {?>

    <li class="page-item"><a class="page-link" onclick="javascript:location.href='<?php echo $ayarcek['ayar_url']; ?>kategori/<?=seo($kategoricekse['kategori_isim']).'/'.$kategoricekse["kategori_id"]?>?sayfa=<?php echo $s; ?>';"><?php echo $s; ?></a></li>
     <?php   }
